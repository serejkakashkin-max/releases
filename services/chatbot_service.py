...
- release_week_control: Центр назначений, релизы без ответственного, доступные/исключенные кандидаты
  ...
  "confidence": "high|medium|low"
    ...
    def _is_release_week_control_query(self, normalized: str) -> bool:
        return (
            ...
            or ("центр" in normalized and "назнач" in normalized)
    ...
    def _is_release_document_batch_candidate(self, item: Dict) -> bool:
        ...
        is_reroll = release_type == "reroll" if release_type else bool((item or {}).get("is_reroll"))
    ...
    def _handle_release_week_control(self) -> Dict:
        try:
            ...
            lines.extend([
                "",
                f"[Открыть Центр назначений]({BASE_PATH}/dashboard/release-monitor/assignment-center)",
            ])
            ...
            return {
                "text": f"Не удалось собрать Центр назначений: {exc}",
                "intent": "release_week_control",
                "suggestions": self._release_work_suggestions(),
    ...
    def _handle_release_week_recommendations(self, session: Optional[ChatContext] = None) -> Dict:
        try:
            ...
            if recommendations and session is not None:
                ...
                lines.extend([
                    "",
                    "Могу применить назначения сразу или пройтись по релизам по одному: согласиться, заменить ФИО или пропустить строку.",
                ])
            ...
            return {
                ...
                "suggestions": (
                    ["Применить все", "Проверить по шагам", "Отмена"]
                    if recommendations
                    else ["Центр назначений", "Релизы недели по ответственному", "Сформировать документы по релизу"]
                ),
    ...
    def _is_apply_week_recommendation_command(self, normalized: str) -> bool:
        return (
            ...
            or any(marker in normalized for marker in ("применить все", "применить выбран", "прими все", "назначь всех", "назначай", "все ок", "остальные ок", "остальное ок"))
        )

    def _is_apply_remaining_week_recommendations_command(self, normalized: str) -> bool:
        return any(
            marker in normalized
            for marker in ("применить все", "прими все", "назначь всех", "назначай всех", "все ок", "остальные ок", "остальное ок")
        )

    def _is_start_week_recommendation_step_review(self, normalized: str) -> bool:
        return normalized in {"внести правки", "правки", "изменить", "проверить по шагам", "по шагам", "пошагово"} or any(
            marker in normalized
            for marker in ("пройтись", "проверим", "проверить каждого", "по одному", "поочеред")
        )

    def _is_week_recommendation_skip_current(self, normalized: str) -> bool:
        return any(marker in normalized for marker in ("пропустить", "пропусти", "не назнач", "оставь пуст", "без назнач"))

    def _is_week_recommendation_accept_current(self, normalized: str) -> bool:
        if any(marker in normalized for marker in ("не соглас", "не подходит", "измени", "замени", "друг")):
            return False
        return normalized in {"да", "ок", "ага", "согласен", "согласна", "принять", "подходит", "оставить"} or any(
            marker in normalized
            for marker in ("соглас", "оставь", "оставляем", "подходит", "как предложил", "как рекоменду")
        )

    def _is_week_recommendation_edit_current(self, normalized: str) -> bool:
        return normalized in {"другой ответственный", "другого", "другой", "заменить", "правка", "внести правку"} or any(
            marker in normalized
            for marker in ("другого ответствен", "другой ответствен", "замени", "изменить ответствен", "ввести фио")
        )
    ...
    def _format_week_recommendation_step_text(self, flow: Dict) -> str:
        recommendations = flow.get("recommendations") or []
        index = int(flow.get("review_index") or 0)
        if not recommendations or index >= len(recommendations):
            return "Рекомендации для пошаговой проверки закончились."
        ...
        backup_text = f"\nРезервный вариант: *{backup}*" if backup else ""
        return (
            f"*Проверка назначений* {index + 1}/{len(recommendations)}\n"
            f"Релиз: *{release_key} / {rov_key}*\n"
            f"AI предлагает: *{responsible}*{backup_text}{reason_text}\n\n"
            "Можно согласиться, выбрать другого ответственного или пропустить строку."
        )

    def _week_recommendation_step_suggestions(self, flow: Dict) -> List[str]:
        return ["Согласен", "Другой ответственный", "Пропустить", "Отмена"]

    def _build_week_recommendation_step_response(self, flow: Dict) -> Dict:
        return {
            "text": self._format_week_recommendation_step_text(flow),
            "intent": "release_week_recommendations",
            "suggestions": self._week_recommendation_step_suggestions(flow),
            "metadata": {
                "type": "release_week_recommendations",
                "state": "step_review",
                "review_index": flow.get("review_index", 0),
                "total": len(flow.get("recommendations") or []),
            },
        }

    def _build_week_recommendation_edit_response(self, flow: Dict) -> Dict:
        recommendations = flow.get("recommendations") or []
        index = int(flow.get("review_index") or 0)
        item = recommendations[index] if 0 <= index < len(recommendations) else {}
        release_key = str(item.get("release_key") or "-").strip()
        rov_key = str(item.get("rov_key") or "без РОВ").strip()
        backup = str(item.get("backup") or "").strip()
        suggestions = ["Пропустить", "Отмена"]
        if backup:
            suggestions.insert(0, f"Назначить {backup}")
        return {
            "text": (
                f"Кого назначить ответственным на *{release_key} / {rov_key}*?\n\n"
                "Пришли фамилию или ФИО из списка ответственных, например `Кашкин` или `Кашкин С.Н.`."
            ),
            "intent": "release_week_recommendations",
            "suggestions": suggestions,
            "metadata": {
                "type": "release_week_recommendations",
                "state": "step_review_edit_name",
                "review_index": index,
            },
        }

    def _finish_week_recommendation_step_review(self, session: ChatContext, flow: Dict) -> Dict:
        approved = flow.get("approved_recommendations") or []
        skipped = flow.get("skipped_recommendations") or []
        flow["state"] = "confirm_after_steps"
        flow["recommendations"] = approved
        session.active_release_flow = flow

        if not approved:
            session.active_release_flow = None
            return {
                "text": "Пошаговая проверка завершена. Все рекомендации пропущены, назначать в таблицу нечего.",
                "intent": "release_week_recommendations",
                "suggestions": ["Центр назначений", "Предложи ответственных по релизам недели"],
                "metadata": {"type": "release_week_recommendations", "state": "step_review_empty", "skipped": skipped},
            }

        lines = [
            "Пошаговая проверка завершена. Итоговый план назначений:",
            "",
            self._format_week_recommendation_plan(approved),
        ]
        if skipped:
            lines.extend(["", f"Пропущено строк: {len(skipped)}."])
        lines.extend(["", "Применить эти назначения в таблицу?"])
        return {
            "text": "\n".join(lines),
            "intent": "release_week_recommendations",
            "suggestions": ["Применить", "Проверить заново", "Отмена"],
            "metadata": {
                "type": "release_week_recommendations",
                "state": "confirm_after_steps",
                "recommendations": approved,
                "skipped": skipped,
            },
        }

    def _handle_week_recommendation_step_reply(
        self,
        message: str,
        session: ChatContext,
        flow: Dict,
        ...
        index = int(flow.get("review_index") or 0)
        approved = list(flow.get("approved_recommendations") or [])
        skipped = list(flow.get("skipped_recommendations") or [])

        if index >= len(recommendations):
            return self._finish_week_recommendation_step_review(session, flow)

        if self._is_apply_remaining_week_recommendations_command(normalized):
            approved.extend(dict(item) for item in recommendations[index:])
            flow["approved_recommendations"] = approved
            flow["review_index"] = len(recommendations)
            return self._finish_week_recommendation_step_review(session, flow)

        current = dict(recommendations[index])
        state = flow.get("state")

        if state == "step_review_edit_name":
            if self._is_week_recommendation_skip_current(normalized):
                skipped.append(current)
            else:
                matched_name = self._match_oplot_name(message)
                if not matched_name:
                    return {
                        "text": (
                            "Не нашел такого ответственного в списке доступных ФИО. "
                            "Пришли фамилию/ФИО еще раз или выбери `Пропустить`."
                        ),
                        "intent": "release_week_recommendations",
                        "suggestions": ["Пропустить", "Отмена"],
                        "metadata": {
                ...
                current["recommended"] = matched_name
                current["reason"] = "ручная правка через чат"
                approved.append(current)
        elif self._is_week_recommendation_edit_current(normalized):
            flow["state"] = "step_review_edit_name"
            session.active_release_flow = flow
            return self._build_week_recommendation_edit_response(flow)
        elif self._is_week_recommendation_skip_current(normalized):
            skipped.append(current)
        else:
            matched_name = self._match_oplot_name(message)
            if matched_name:
                ...
                current["reason"] = "ручная правка через чат"
                approved.append(current)
            elif self._is_week_recommendation_accept_current(normalized):
                approved.append(current)
            else:
                return {
                    "text": (
                        "Не понял выбор по текущему релизу. Ответь `Согласен`, `Пропустить` "
                        "или напиши фамилию/ФИО из списка ответственных."
                    ),
                    "intent": "release_week_recommendations",
                    "suggestions": self._week_recommendation_step_suggestions(flow),
                    "metadata": {
                        "type": "release_week_recommendations",
                        "state": "step_review_parse_error",
                        "review_index": index,
                    },
                }

        flow["approved_recommendations"] = approved
        flow["skipped_recommendations"] = skipped
        flow["review_index"] = index + 1
        flow["state"] = "step_review"
        session.active_release_flow = flow

        if flow["review_index"] >= len(recommendations):
            return self._finish_week_recommendation_step_review(session, flow)
        return self._build_week_recommendation_step_response(flow)
    ...
    def _handle_week_recommendation_reply(self, message: str, session: ChatContext) -> Dict:
        ...
        if not recommendations:
            ...
            return {
                ...
                "intent": "release_week_recommendations",
                "suggestions": ["Предложи ответственных по релизам недели", "Центр назначений"],
                "metadata": {"type": "release_week_recommendations", "state": "empty"},
            }

        state = flow.get("state")
        if state in {"step_review", "step_review_edit_name"}:
            return self._handle_week_recommendation_step_reply(message, session, flow, recommendations, normalized)

        if normalized == "проверить заново" and state == "confirm_after_steps":
            recommendations = flow.get("original_recommendations") or recommendations
            flow["recommendations"] = recommendations
            flow["state"] = "step_review"
            flow["review_index"] = 0
            flow["approved_recommendations"] = []
            flow["skipped_recommendations"] = []
            flow["original_recommendations"] = recommendations
            session.active_release_flow = flow
            return self._build_week_recommendation_step_response(flow)

        if self._is_start_week_recommendation_step_review(normalized):
            original_recommendations = flow.get("original_recommendations") or recommendations
            flow["recommendations"] = original_recommendations
            flow["state"] = "step_review"
            flow["review_index"] = 0
            flow["approved_recommendations"] = []
            flow["skipped_recommendations"] = []
            flow["original_recommendations"] = original_recommendations
            session.active_release_flow = flow
            return self._build_week_recommendation_step_response(flow)
        ...
        if unknown_parts and not edits and not skipped and not self._is_apply_week_recommendation_command(normalized):
            return {
                "text": (
                    ...
                    "Можно нажать `Проверить по шагам`, и я последовательно спрошу по каждому релизу. "
                    "Либо пришли правки в формате `ключ релиза + ФИО`, например `EMRM-12345 Кашкин С.Н.`."
                ),
                "intent": "release_week_recommendations",
                "suggestions": ["Проверить по шагам", "Применить все", "Отмена"],
        ...
        if edits or skipped:
            ...
            if not self._is_apply_week_recommendation_command(normalized):
                return {
                    ...
                    "intent": "release_week_recommendations",
                    "suggestions": ["Применить все", "Проверить по шагам", "Отмена"],
        ...
        if self._is_apply_week_recommendation_command(normalized) or edits or skipped:
            ...
            return {
                "text": "\n".join(lines),
                "intent": "release_week_recommendations",
                "suggestions": ["Центр назначений", "Релизы недели по ответственному", "Сформировать документы по релизу"],
        ...
        return {
            "text": (
                ...
                f"{self._format_week_recommendation_plan(recommendations)}\n\n"
                "Напиши `применить все`, `проверить по шагам` или `отмена`."
            ),
            "intent": "release_week_recommendations",
            "suggestions": ["Применить все", "Проверить по шагам", "Отмена"],
    ...
    def _handle_current_week_release_report(self) -> Dict:
        try:
            ...
            return {
                ...
                "intent": "release_current_week_report",
                "suggestions": ["Центр назначений", "Предложи ответственных по релизам недели", "Выгрузить таблицу релизов в Confluence"],
    ...
    def _handle_release_week_assignee_query(self, message: str, session: Optional[ChatContext] = None) -> Dict:
        ...
        for suggestion_item in matched_items:
            ...
            if len(release_suggestions) >= 6:
                break
        suggestions = release_suggestions + ["Подготовить документы по всем этим релизам", "Выгрузить таблицу релизов в Confluence", "Центр назначений", "Что ты умеешь"]
        if session is not None:
            session.active_release_flow = {
    ...
    def _normalize_responsible_candidates(self, values) -> List[str]:
    ...
    def _get_release_flow_responsibles(self, flow: Dict) -> List[str]:
        candidates = self._normalize_responsible_candidates(flow.get("responsibles") or [])
        if candidates:
            return candidates

        row_key = str(flow.get("row_key") or flow.get("release_key") or "").strip()
        item = self._get_release_row_by_row_key(row_key) if row_key else None
        if not item and flow.get("release_key"):
            rows = self._find_release_rows_for_key(str(flow.get("release_key") or ""))
            item = rows[0] if rows else None
        if item:
            candidates = self._normalize_responsible_candidates(item.get("psi_responsibles") or [])
            if candidates:
                flow["responsibles"] = candidates
        return candidates

    def _select_responsible_candidate(self, candidates: List[str], message: str) -> str:
        clean_candidates = self._normalize_responsible_candidates(candidates)
        ...
        if number_match:
            index = int(number_match.group(1)) - 1
            if 0 <= index < len(clean_candidates):
                return clean_candidates[index]

        for candidate in clean_candidates:
            candidate_normalized = self._normalize_command_text(candidate)
            if normalized == candidate_normalized:
                return candidate

        for candidate in clean_candidates:
            candidate_normalized = self._normalize_command_text(candidate)
            surname = candidate_normalized.split(" ", 1)[0]
            if surname and (normalized == surname or surname in normalized or normalized in candidate_normalized):
                return candidate
    ...
    def _build_release_doc_template_not_found_response(self, release_key: str) -> Dict:
        release_key = str(release_key or "").strip()
        release_text = f" для *{release_key}*" if release_key else ""
        return {
            "text": (
                f"К сожалению, шаблоны документов{release_text} сейчас не найдены. "
                "Открой Блок релизов, проверь данные строки и повтори формирование документов из таблицы."
            ),
            "intent": "release_document_flow",
    ...
    def _build_zni_reporter_choice_response(self, flow: Dict) -> Dict:
        release_key = str(flow.get("release_key") or "").strip()
        rov_key = str(flow.get("rov_key") or "без РОВ").strip()
        candidates = self._get_release_flow_responsibles(flow)
        lines = [
            f"По *{release_key} / {rov_key}* назначено несколько ответственных.",
            "Кого указать автором задачи OPLOT?",
            "",
        ]
        for index, name in enumerate(candidates, 1):
            lines.append(f"{index}. `{name}`")
        lines.append("")
        lines.append("Можно нажать вариант ниже или прислать ФИО вручную.")

        suggestions = candidates[:5] + ["Отмена"]
        return {
            "text": "\n".join(lines),
            "intent": "release_document_flow",
            "suggestions": suggestions,
    ...
    def _set_release_doc_flow_template(self, flow: Dict, detection: Dict, original_message: str = "") -> Optional[Dict]:
        try:
            ...
            if detection.get("found"):
                ...
                flow["playbooks_required"] = release_uses_playbooks(flow["release_full"], flow["category"])
                return None
            if candidates:
    ...
    def _ensure_release_document_jira_gate(self, session: ChatContext) -> Optional[Dict]:
        ...
        if flow.get("template_not_found_after_jira") and not self._template_signature(flow):
            if flow.get("batch_context"):
                return self._skip_release_document_batch_item(
                    session,
                    flow.get("batch_context") or {},
                    {
                        "release_key": release_key,
                        "rov_key": flow.get("rov_key", ""),
                        "row_key": flow.get("row_key", ""),
                    },
                    "шаблоны документов не найдены после проверки Jira",
                )
            return self._build_release_doc_template_not_found_response(release_key)
    ...
    def _apply_template_candidate_to_flow(self, flow: Dict, candidate: Dict, release_uses_playbooks_func) -> None:
        ...
        flow["playbooks_required"] = release_uses_playbooks_func(release_full, flow["category"])
    ...
    def _prepare_release_document_flow_for_item(
        ...
        *,
        ...
        try:
            ...
            if str(item.get("ke_id") or "").strip() and not detection.get("found") and not preliminary_candidates:
                if batch_context:
                    return self._skip_release_document_batch_item(
                        session,
                        batch_context,
    ...
    def _format_release_batch_item_label(self, item: Dict) -> str:
        release_key = str(item.get("release_key") or item.get("row_key") or "").strip()
        rov_key = str(item.get("rov_key") or "").strip()
        if release_key and rov_key:
            return f"{release_key} / {rov_key}"
        return release_key or rov_key or "релиз"

    def _append_release_document_batch_skip(self, batch_context: Dict, item: Dict, reason: str) -> None:
        skipped = list(batch_context.get("skipped") or [])
        skipped.append({
            "release_key": str(item.get("release_key") or "").strip(),
            "rov_key": str(item.get("rov_key") or "").strip(),
            "row_key": str(item.get("row_key") or item.get("release_key") or "").strip(),
            "reason": str(reason or "").strip() or "пропущен",
        })
        batch_context["skipped"] = skipped

    def _build_release_document_batch_finish_response(self, batch_context: Dict, prefix_text: str = "") -> Dict:
        completed = list(batch_context.get("completed") or [])
        skipped = list(batch_context.get("skipped") or [])
        total = int(batch_context.get("total") or len(batch_context.get("items") or []))
        lines = []
        if prefix_text:
            lines.extend([prefix_text, ""])

        lines.append(
            f"Пакетное оформление завершено. "
            f"Готово: {len(completed)}, пропущено: {len(skipped)}, всего: {total}."
        )

        if completed:
            lines.extend(["", "*Готовые архивы:*"])
        ...
        if skipped:
            lines.extend(["", "*Пропущено:*"])
            for item in skipped[:10]:
                label = self._format_release_batch_item_label(item)
                lines.append(f"- {label}: {item.get('reason') or 'пропущен'}")
            if len(skipped) > 10:
                lines.append(f"- и еще {len(skipped) - 10}")
    ...
    def _skip_release_document_batch_item(
        self,
        session: ChatContext,
        batch_context: Dict,
        item: Dict,
        ...
        self._append_release_document_batch_skip(batch_context, item, reason)
        skipped_label = self._format_release_batch_item_label(item)
        ...
        if next_index < len(batch_items):
            ...
            next_response["text"] = (
                f"Пропускаю {skipped_label}: {reason}.\n\n"
        ...
        session.active_release_flow = None
        return self._build_release_document_batch_finish_response(
            batch_context,
            prefix_text=f"Пропускаю {skipped_label}: {reason}.",
        )
    ...
    def _start_release_document_batch(self, session: ChatContext) -> Dict:
        ...
        if not batch_items:
            ...
            if skipped_items:
                ...
                return {
                    ...
                    "intent": "release_document_batch",
                    "suggestions": ["Релизы недели по ответственному", "Сформировать документы по релизу", "Центр назначений"],
                    "metadata": {
                        "type": "release_document_batch",
        ...
        response.setdefault("metadata", {})["batch"] = {
            "index": int(batch_context.get("index") or 0),
            "total": len(batch_items),
            "skipped_count": len(skipped_items),
            "skipped": list(batch_context.get("skipped") or []),
        }
    ...
    def _handle_release_document_flow_reply_v2(self, message: str, session: ChatContext) -> Dict:
        ...
        if state == "zni_reporter_requested":
            candidates = self._get_release_flow_responsibles(flow)
            reporter = self._select_responsible_candidate(candidates, message)
            if not reporter:
                return self._build_zni_reporter_choice_response(flow)
            flow["zni_reporter"] = reporter
            flow["state"] = "zni_confirm"
            session.active_release_flow = flow
            return self._generate_release_documents_from_flow(session, create_zni=True)
    ...
    def _generate_release_documents_from_flow(self, session: ChatContext, create_zni: bool = False) -> Dict:
        ...
        try:
            ...
            if create_zni and not flow.get("zni_key"):
                ...
                zni_reporter = str(flow.get("zni_reporter") or "").strip()
                responsibles = self._get_release_flow_responsibles(flow)
                if len(responsibles) > 1 and not zni_reporter:
                    flow["state"] = "zni_reporter_requested"
                    flow["pending_create_zni"] = True
                    session.active_release_flow = flow
                    return self._build_zni_reporter_choice_response(flow)
                issue_result = create_release_monitor_zni(flow.get("row_key", ""), reporter=zni_reporter)
            ...
            if create_zni and (
                ("нескольк" in error_text and "ответствен" in error_text)
                or ("РЅРµСЃРєРѕР»СЊРє" in error_text and "РѕС‚РІРµС‚СЃС‚РІРµРЅ" in error_text)
            ):
                flow["state"] = "zni_reporter_requested"
                session.active_release_flow = flow
                return self._build_zni_reporter_choice_response(flow)
    ...
    def _handle_unknown(self) -> Dict:
        ...
        return {
            'text': (
                ...
                "Можно так: `релизы недели за Ивановым`, `оформи документы по EMRM-12345`, `центр назначений`, `найди задачу OPLOT-12345`."
            ),
            'suggestions': self.get_default_suggestions(),
    ...
    def _handle_show_capabilities(self) -> Dict:
        ...
        return {
            'text': (
                ...
                "1. Работать с релизами: показать релизы недели по ответственному, собрать Центр назначений, подготовить релизную статистику и предложить ответственных.\n"
                "Примеры: `Какие релизы текущей недели закреплены за Ивановым?`, `Покажи Центр назначений`, `Предложи ответственных по релизам недели`.\n\n"
    ...
    def _handle_release_statistics(self, params: Dict, dashboard_context: Dict = None, original_message: str = '') -> Dict:
        ...
        try:
            ...
            return {
                'text': text,
                'suggestions': ["Центр назначений", "Отчет релизов текущей недели", "Выгрузить таблицу релизов в Confluence"],
                'metadata': {
