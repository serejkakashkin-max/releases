...
МПР использует no-sidebar core shell, namespace `.oplot-mpr` и общие `--oplot-dna-*` tokens. Одностраничный контракт остаётся неизменным: XLSX/CSV → предварительная проверка → выбор комплектов → DOCX или ZIP. Application URLs передаются внешнему first-party JavaScript только через server-rendered prefix-safe JSON; package modal использует локальный Tabler API и не вводит глобальный adapter. Presentation-миграция не изменяет реальные MPR-шаблоны или генератор.

Текущий security contract МПР включает серверную allowlist расширений, разрешение шаблона внутри выделенного root и общий Flask request limit 50 MiB. Отдельного MPR CSRF, MIME validation и per-file limit сейчас нет; это зафиксированный долг финальной security stabilization, а не уже реализованная защита.
...
## Административный контур и Current Week (PR 8A)

«СУП-параметры» используют no-sidebar core shell и namespace `.oplot-sup-admin`. Восемь существующих вкладок, их порядок, query `tab/view/token`, aliases, legacy SUP token и отдельная admin session/CSRF остаются пользовательскими и security-контрактами. First-party JavaScript получает application URL только из server-rendered prefix-safe JSON и сохраняет polling 15 секунд, active polling 2 секунды и backoff до 60 секунд. Presentation service не читает и не изменяет административное состояние или VA repositories.

«Релизы текущей недели» остаются единым автономным HTML для live-страницы и сохранённого отчёта. CSS, JavaScript и SVG темы встроены в документ: `/static/**`, CDN, внешние шрифты и Bootstrap Icons не требуются. Миграция меняет только presentation на Oplot DNA и не меняет период, фильтрацию, сортировку, статистику, десять колонок, installed/hidden, reroll/hotfix, refresh, сохранение отчёта и route/cache contracts.
