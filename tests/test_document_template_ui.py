...
class DocumentTemplateUiTests(unittest.TestCase):
    ...
    def test_dtc_css_and_filter_script_keep_modal_and_variant_layout_stable(self):
        ...
        self.assertRegex(css, r"\.oplot-dtc\s*\{[^}]*min-width:\s*0")
        self.assertRegex(css, r"\.oplot-dtc\s*\{[^}]*overflow-x:\s*hidden")
        self.assertRegex(css, r"\.oplot-dtc\s*\{[^}]*background:\s*transparent")
        self.assertRegex(css, r"html:has\(> body\.oplot-dtc\)\s*\{[^}]*overflow-x:\s*hidden")
        self.assertRegex(css, r"\.oplot-dtc \.oplot-page\s*\{[^}]*background:\s*transparent")
        self.assertRegex(css, r"\[data-theme=\"light\"\] \.oplot-dtc \.oplot-page\s*\{[^}]*background:\s*transparent")
        self.assertIn(".oplot-dtc .oplot-dtc-table-wrap { max-width: 100%; overflow-x: auto;", css)
        ...
        self.assertIn('var DELETE_SCROLL_KEY = "oplot-dtc-delete-return"', script)
        self.assertIn("window.location.pathname + window.location.search", script)
        self.assertIn("window.location.reload()", script)
        self.assertRegex(css, r"#kit-create-modal \.modal-content > form\s*\{[^}]*max-height:\s*calc\(100dvh - 2rem\)")
        self.assertRegex(css, r"#kit-create-modal \.modal-body\s*\{[^}]*overflow-y:\s*auto")
        self.assertRegex(css, r"#kit-create-modal \.modal-footer\s*\{[^}]*flex:\s*0 0 auto")
        self.assertIn("window.scrollTo(0, Number(deleteReturn.scrollY))", script)
